...
metric2 = PyRouge(multi_ref_mode='best')

dataloader = DataLoader(split_dataset["val"], batch_size=64, num_workers=16)
...
for i, batch in enumerate(dataloader):
  ...
  with torch.no_grad():
    output_sequences = model.generate(**model_inputs, num_beams=10)
  predictions += tokenizer.batch_decode(output_sequences, skip_special_tokens=True)

predictions_bleu = [i.split() for i in predictions]
references_bleu = [[i.split() for i in j] for j in split_dataset["val"]["labels"]]
predictions_rouge = [[i.split()] for i in predictions]
references_rouge = [[[i.split()] for i in j] for j in split_dataset["val"]["labels"]]

print(metric.compute(predictions=predictions_bleu, references=references_bleu))
print(metric2.evaluate_tokenized(predictions_rouge, references_rouge))